...
def download_and_save_image(raw_url, product_sku, suffix):
    ...
    try:
        # Визначаємо, як файл буде називатися в хмарі (його ID)
        cloud_file_name = f"silpo_products/{product_sku}_{suffix}"

        # --- КРОК 1. ПЕРЕВІРКА В CLOUDINARY ---
        try:
            # Пробуємо знайти інформацію про файл у хмарі
            ...
            # Якщо Cloudinary видав помилку NotFound — файлу немає, йдемо далі
            pass

        # --- КРОК 2. ЯКЩО ФАЙЛУ НЕМАЄ, СКАЧУЄМО ЛОКАЛЬНО ---
        ext = raw_url.split('.')[-1]
